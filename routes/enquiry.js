...
// GET active customers (name + Location for Groupname)
router.get("/getcustomers", authMiddleware, async (req, res) => {
  try {
    const rows = await pool.query(
      `SELECT customername, Location FROM customer WHERE status='Active' ORDER BY customername ASC`,
    );
    res.json(
      rows.map((r) => ({ customername: r.customername, location: r.Location })),
    );
  } catch (err) {
...
// GET customer info (type + country + groupname + currency) by customer name
router.get("/getcustomerinfo", authMiddleware, async (req, res) => {
  const { customername } = req.query;
  try {
    const rows = await pool.query(
      `SELECT customercountry, customertype, Location, Currency
    ...
    res.json({
      country: rows[0].customercountry,
      custtype: rows[0].customertype,
      location: rows[0].Location, // → Groupname
...
// GET opportunity stages filtered by quote stage
...
// GET end use description by industry
...
router.get("/getff", authMiddleware, async (req, res) => {
  try {
    const rows = await pool.query(
      `SELECT Data FROM quotedata WHERE Type='Facingfactory' AND Status='Active' ORDER BY Data ASC`,
    );
    res.json(rows.map((r) => r.Data));
  } catch (err) {
    res.status(500).json({ message: "Server error" });
...
// GET products (optionally filtered by facing factory)
router.get("/getproducts", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (facingfactory) {
      rows = await pool.query(
        `SELECT Products FROM product WHERE status='Active' AND FacingFactory=? ORDER BY Products ASC`,
        [facingfactory],
      );
    } else {
      rows = await pool.query(
...
router.get("/fetchsno", authMiddleware, async (req, res) => {
  try {
    ...
    let sno = rows[0].cnt === 0 || rows[0].sno === null ? 1 : rows[0].sno + 1;
    const paddedSno = String(sno).padStart(4, "0");
    const quoteNumber = `R${currentYear}${paddedSno}`;
    res.json({ sno, quoteNumber });
  } catch (err) {
    res.status(500).json({ message: "Server error" });
  }
});
...
/* ════════════════════════════════════════
   GET ALL ENQUIRIES (table listing)
════════════════════════════════════════ */
...
/* ════════════════════════════════════════
   GET SINGLE ENQUIRY by Quotenumber
════════════════════════════════════════ */
...
/* ════════════════════════════════════════
   ADD ENQUIRY  POST /api/enquiry/submit
════════════════════════════════════════ */
router.post("/submit", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // ── Insert main quoteregister record ──
    await conn.query(
    ...
    // ── Auto-add custom RFQType if not in quotedata ──
    if (RFQType) {
      ...
      if (!ex)
        await conn.query(
          `INSERT INTO quotedata (Data, Type) VALUES (?, 'Opportunitytype')`,
          [rfqUpper],
        );
    }

    // ── Auto-add custom EndIndustry ──
    if (EndIndustry) {
      const [ex] = await conn.query(
        `SELECT Sno FROM endindustry WHERE Industry=?`,
        [EndIndustry],
      );
      if (!ex)
        await conn.query(`INSERT INTO endindustry (Industry) VALUES (?)`, [
          EndIndustry,
        ]);
    }

    // ── Auto-add custom FacingFactory ──
    if (Facingfactory) {
      ...
      if (!ex)
        await conn.query(
          `INSERT INTO quotedata (Data, Type) VALUES (?, 'Facingfactory')`,
          [ffUpper],
        );
    }

    // ── Insert into quotetimeline (one row per product) ──
...
/* ════════════════════════════════════════
   UPDATE ENQUIRY  PUT /api/enquiry/:quotenumber
════════════════════════════════════════ */
router.put("/:quotenumber", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // ── Check exists + get current stage ──
    ...
    if (
      (existingStage === "TECHNICAL OFFER" &&
        ["PRICED OFFER", "ENQUIRY"].includes(Quotestage?.toUpperCase())) ||
    ...
    // ── Copy current record to quoteregisterhistory ──
    ...
    // ── Update main quoteregister record ──
    await conn.query(
...
/* ════════════════════════════════════════
   GENERATE QUOTE  POST /api/enquiry/generate-quote
════════════════════════════════════════ */
router.post("/generate-quote", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Check if timeline already generated
    ...
    // Update quotetimeline — set Enquiry='Y'
    await pool.query(
    ...
    // If no timeline row exists yet, insert it per product
